...
export default function Memoria() {
    ...
    return (
        <div className="flex flex-col items-center justify-center p-6 min-h-screen bg-gray-100 font-poppins">
            {!started && (
                <div className="mb-6 bg-white p-8 rounded-xl shadow-md w-full max-w-lg">
                    <h2 className="text-3xl font-bold text-center mb-8 text-gray-800">Selecione o Tema</h2>
                    
                    <div className="grid grid-cols-2 gap-4 mb-8">
                        {(["aleatorio", "animais", "cores", "comidas"] as const).map(cat => (
                            <label key={cat} className={`flex flex-col items-center justify-center p-4 border-2 rounded-xl cursor-pointer transition-all ${
                                categoriaSelecionada === cat ? 'border-green-500 bg-green-50 transform scale-105 shadow-sm' : 'border-gray-200 hover:bg-gray-50'
                            }`}>
                                <input
                                    name="theme-radio"
                                    type="radio"
                                ...
                                />
                                <span className={`font-semibold text-lg capitalize ${categoriaSelecionada === cat ? 'text-green-700' : 'text-gray-600'}`}>
                    ...
                    <button
                        type="button"
                        onClick={iniciarJogo}
                        className="w-full bg-green-500 hover:bg-green-600 text-white font-bold py-4 px-4 rounded-xl text-lg flex items-center justify-center transition-colors cursor-pointer shadow-md hover:shadow-lg"
                    >
                        Iniciar Jogo
                    </button>
                </div>
